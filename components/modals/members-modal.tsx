...
import { FullServerInfo } from "@/types";
...
const MembersModal = () => {
  ...
  const [loadingId, setLoadingId] = useState(""); // When a request is made for a specific ID, such as changing a user's role or kicking them, we display a spinner.
        ...
        <ScrollArea className={"mt-8 max-h-[420px] pr-6"}>
          {server?.members?.map((member) => (
            <div className={"flex items-center gap-x-2 mb-6"} key={member.id}>
              ...
              <div className="texs-xs font-semibold flex items-center gap-x-1">
                {member.profile.name}
